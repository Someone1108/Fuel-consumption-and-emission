...
# Adjust top planes dataset with seat capacity and calculate lifetime cost, emissions, and fuel consumption
top_planes <- top_planes %>%
  mutate(NumberOfSeats = seat_capacity[`Aircraft Type`],  # Add seat capacity to the planes
         ...
         LifetimeCost = Cost * lifetime_distance_plane)  # Calculate lifetime cost

# Calculate lifetime fuel consumption, emissions, and cost for top 3 cars
top_cars <- top_cars %>%
  mutate(LifetimeFuelConsumption = FuelConsumption * lifetime_distance_car,
         LifetimeEmissions = Emissions * lifetime_distance_car,
         LifetimeCost = Cost * lifetime_distance_car) %>%
...
combined_lifetime_data <- rbind(
  data.frame(Vehicle = top_planes$`Aircraft Type`, Type = "Plane", 
             ...
             LifetimeCost = top_planes$LifetimeCost),
  data.frame(Vehicle = top_cars$Model, Type = "Car", 
             ...
             LifetimeCost = top_cars$LifetimeCost)
)

# Reshape data to long format for ggplot, now including LifetimeCost
combined_long <- combined_lifetime_data %>%
  pivot_longer(cols = c(LifetimeFuelConsumption, LifetimeEmissions, LifetimeCost),
               names_to = "Metric", values_to = "Value")

# Plot grouped bar chart including LifetimeCost with formatted y-axis
ggplot(combined_long, aes(x = Vehicle, y = Value, fill = Type)) +
  ...
  labs(title = "Lifetime Fuel Consumption, CO2 Emissions, and Cost of Top 3 Cars and Planes",
